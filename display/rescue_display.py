...
class RescueDisplay:
    ...
    def _build(self, fd: FrameData) -> np.ndarray:
        ...
        if self._show_thermal and self._main_view_idx != 1:
            ...
            # ── Battery info displayed ABOVE the PiP window ───────────
            ...
            _txt(canvas, f"BAT  {volts_pip:.1f}V  {batt_rem_pip}%", (pip_x + 4, pip_y - 72), 0.48, WHITE)
            _txt(canvas, f"AMP  {current_pip:.1f} A",               (pip_x + 4, pip_y - 48), 0.48, WHITE)
